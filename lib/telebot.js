...
class TeleBot {
    ...
    constructor(cfg) {
        ...
        const userPluginList = this.usePlugins.filter((item) => this.defaultPlugins.indexOf(item) < 0);
        for (let pluginName of this.defaultPlugins.concat(userPluginList)) {
            this.plug(require(this.pluginFolder + pluginName));
        }
    ...
    use(module) {
        console.log('[bot.warning] Use .plug() instead of .use()');
        return this.plug(module);
    }
