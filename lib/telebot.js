...
class TeleBot {
  ...
  connect() {
    ...
    if (this.webhook) {
      ...
      const cert = fs.readFileSync(this.webhook.cert);
      return this.setWebhook(url, cert).then(x => {
        console.log(`[bot.webhook] set to "${url}"`, x);
        return webhook.call(this, this.webhook);
      }).catch(error => {
    ...
    this.setWebhook().then(data => {
      ...
      if (data.description == 'Webhook was deleted')
        console.log('[bot.webhook] webhook has deleted');
  ...
  getUpdate() {
    ...
    }).then(body => this.receiveUpdate(body));
  }
  receiveUpdate(body) {
    // Receive bot update
    ...
    return new Promise((resolve, reject) => {
      this.event('update', data).then(output => {
        ...
        let mod = this.modRun('update', { data, me });
        data = mod.data, me = mod.me;
        ...
        for (let update of data) {
          ...
          if (update['inline_query']) {
            ...
            for (let type of MESSAGE_TYPES) {
              ...
              me.type = type;
              // Run message modifiers
              let mod = this.modRun(`message.${ type }`, { msg, me });
              msg = mod.msg;
              me = mod.me;
              // Send type event
