...
const DEFAULT_MODULES = ['regExpMessage', 'shortReply'];
...
class TeleBot {
  ...
  constructor(cfg) {
    ...
    this.useModules = cfg.useModules !== undefined ? (cfg.useModules || []) : DEFAULT_MODULES;
    ...
    this.allowedUpdates = typeof poll.allowedUpdates == 'string' || Array.isArray(poll.allowedUpdates) ? poll.allowedUpdates : [];
    ...
    // Use default build-in modules
    if (this.useModules) {
      for (let name of this.useModules) {
        this.use(require(`../modules/${ name }`));
      }
    }
  ...
  use(module) {
    return module.call(this, this, this.cfg.modules || {});
  }
