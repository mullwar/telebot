...
class TeleBot {
  ...
  constructor(cfg) {
    ...
    if (!poll) {
      if (cfg.pooling) {
        ...
        poll = {};
        // Set cfg.polling
        for (let name of ['limit', 'timeout', 'retryTimeout', 'allowed_updates']) {
          poll[name] = cfg[name];
        }
    ...
    this.allowed_updates = typeof poll.allowed_updates == 'string' || Array.isArray(poll.allowed_updates) ? poll.allowed_updates : undefined;
  ...
  getUpdates(offset=this.updateId, limit=this.limit, timeout=this.timeout, allowed_updates=this.allowed_updates) {
