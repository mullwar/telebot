...
class TeleBot {
  ...
  constructor(cfg) {
    ...
    if (!pul) {
      ...
      // cfg.sleep renamed to cfg.pooling.interval
      pul.interval = cfg.sleep;
    }

    this.limit = Number(pul.limit) || 100;
    this.interval = Number(pul.interval) || 1000;
    this.timeout = pul.timeout >= 0 ? pul.timeout : 0;
    this.retryTimeout = pul.retryTimeout >= 0 ? pul.retryTimeout : 5000;
  ...
  connect() {
    ...
    // Set webhook
    if (this.webhook) {
      const url = this.webhook.url;
      return this.setWebhook(url).then(x => {
    ...
    this.loopFn = setInterval(x => {
      ...
      // Skip processing on false pool flag
      if (!f.pool) return;
      ...
      this.getUpdates().then(x => {
        ...
        setTimeout(x => (f.pool = true), this.retryTimeout);
